...
df_total_cereals_mean_5yr <- df_total_cereals %>%
  filter(Year >= (CurrentYear-5) & Year <= (CurrentYear-1)) %>%
...
Crop <- ggplot(df_total_cereals, aes(Year)) +
  ...
  geom_segment(
    ...
    aes(
      x = (CurrentYear-5), xend = (CurrentYear),
  ...
  # Cereals production: full line (solid)
  geom_line(
    data = df_total_cereals,
    aes(y = Cereals_Production, color = "Total cereals production", group = 1),
    size = 2,
    linetype = "solid"
  )+
...
Crop <- ggplot(df_barley, aes(Year)) +
  ...
  geom_line(
    data = df_barley,
    aes(y = S_Barley_Production, color = "Spring barley production", group = 1),
    size = 2,
    linetype = "solid"
  ) +
